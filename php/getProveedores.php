...
if($conn){
    ...
    $query="SELECT A.NOMBRE_COMERCIAL, A.ID AS ID_ESTABLECIMIENTO, A.URL_IMAGEN,B.NOMBRE AS CIUDAD FROM AC_ESTABLECIMIENTO A, AC_P_CIUDAD B, AC_MERCADO_OFERENTE C, AC_TIPO_SERVICIO_E D WHERE A.ID = C.ESTABLECIMIENTO_COMERCIO AND A.CIUDAD = B.CODIGO AND C.MERCADO_OBJETIVO=$mercadoObjetivo AND A.ID = D.ESTABLECIMIENTO_COMERCIO AND D.SERVICIO=$tipoServicio";
